...
class LPKTNet(nn.Module):
    def __init__(self, exercise_num, skill_num, stu_num, ans_time_num, interval_time_num, option_num, d_k, d_a, d_e, q_matrix):
        '''
        ...
        '''Option Embedding'''
        self.option_embed = nn.Embedding(option_num + 1, d_k)
        ...
        # Option Layer
        # input = h(t-1) + learning gain (t) + option_weight
        self.option_gate = nn.Linear(self.d_k * 3, self.d_k)
        torch.nn.init.xavier_normal_(self.option_gate.weight)

        # Predicting Layer
        # input = exercise (t+1) + h (t)
        self.predicting_layer = nn.Linear(self.d_k * 2, self.d_k)
    ...
    def forward(self, exercise_id,  ans_time, interval_time, option_value, answer_value):
        ...
        option = self.option_embed(option_value)  # batch_size * sequence * d_k
        ...
        for t in range(sequence_len - 1):
            ...
            # h_pre:[batch_size, knowledge, d_k]
            # LG:[batch_size, d_k] -> repeat operation -> [batch_size, knowledge, d_k]
            # option: [batch_size, d_k] -> repeat operation -> [batch_size, knowledge, d_k]
            option_factor = self.option_gate(torch.cat(
                (
                    h_pre.float(),
                    LG.repeat(1, self.skill_num).view(
                        batch_size, -1, self.d_k),
                    option[:, t].repeat(1, self.skill_num).view(
                        batch_size, -1, self.d_k)
                ), 2
            )
            )
            # [batch_size, knowledge, d_k]
            option_factor = self.sigmoid(option_factor)

            '''Unpdating knwoledge state'''
            # h_pre [batch_size, skill, d_k]
            h = LG_tilde + forget_factor * h_pre + option_factor * h_pre
            h = h.to(torch.float64)

            '''Predicting module'''
            h_tilde = knowledge_vector.bmm(h).view(
                batch_size, -1)  # [batch_size, dk)
            prediction[:, t+1] = self.sigmoid(self.predicting_layer(
                torch.cat((exercise[:, t+1], h_tilde.float()), 1))).sum(1) / self.d_k
