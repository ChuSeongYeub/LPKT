...
class LPKTNet(nn.Module):
    def __init__(self, exercise_num, skill_num, stu_num, ans_time_num, interval_time_num, option_num, d_k, d_a, d_e, d_o, q_matrix):
        '''
        ...
        '''Option Embedding'''
        self.option_embed = nn.Embedding(option_num + 1, d_o)
        ...
        self.learning_embed_layer = nn.Linear(
            self.d_e + self.d_k + self.d_a + self.d_o, self.d_k)
    ...
    def forward(self, exercise_id,  ans_time, interval_time, option_value, answer_value):
        ...
        option = self.option_embed(option_value)  # batch_size * sequence * d_o
        ...
        learning_emd = self.learning_embed_layer(torch.cat(
            (exercise, ans_time, answer, option), 2))  # [batch_size, sequence, d_k]
